...
- Keyword search matches English expression titles through Virtuoso's `bif:contains` full-text phrase index (multi-word input is quoted as a single phrase), with a CELEX arm chosen by the keyword's shape: a whole CELEX matches its bounded family of exact literals (itself, `(01)`–`(20)`, `R(01)`–`R(20)`, and the `_INF`/`_RES`/`_SUM`/`_EXT` records) plus its linked corrigenda, a partial CELEX through prefix terms on the CELEX full-text index with a confirming substring test (one opening with letters no digit follows, such as `R(01)`, or with a year and a slash, such as `2024/01469`, by a substring scan of every CELEX literal; a bare year or a fragment opening mid-year or mid-number matches titles only), and a keyword with no digit or with a character no CELEX holds matches titles only; a keyword with no letter or digit is rejected as `invalid_keyword`; there is no full-text search of act body text
...
- a year and a slash (`2024/01469`): the substring scan, since only the C-sector forms hold a `/` (1,907 `C/{year}/{number}` literals such as `C/2024/01469`, plus `C2017/111/07`), and a sector term cannot complete a year that stands as a word of its own;
...
- letters no digit follows (`R(01)`, `ROU_202405`, `C/2024/0146`): the substring scan of every CELEX literal, `FILTER(CONTAINS(STR(?kwCelex), K))`, since those letters can sit anywhere after the type code;
