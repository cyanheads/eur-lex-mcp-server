...
- Keyword search matches English expression titles through Virtuoso's `bif:contains` full-text phrase index (multi-word input is quoted as a single phrase), with a CELEX arm chosen by the keyword's shape: a whole CELEX matches its bounded family of exact literals (itself, `(01)`–`(20)`, `R(01)`–`R(20)`, and the `_INF`/`_RES`/`_SUM`/`_EXT` records) plus its linked corrigenda, a partial CELEX through prefix terms on the CELEX full-text index with a confirming substring test (one opening with letters no digit follows, such as `R(01)`, by a substring scan of every CELEX literal; a fragment opening mid-year or mid-number matches titles only), and a keyword with no digit or with a character no CELEX holds matches titles only; a keyword with no letter or digit is rejected as `invalid_keyword`; there is no full-text search of act body text
...
- anything else, including four digits before a letter that are no year (`016R0679`, `0679`, `0679R`, `0680DEU`, `9999R0679`): no CELEX arm, titles only, a recall change for fragments opening mid-number.
