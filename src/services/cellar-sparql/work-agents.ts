...
import { resolveCorporateBodyLabel } from './cdm-labels.js';
 ...
 * role. The person name and the national-court name are read through separate
 * `OPTIONAL` variables; CELLAR leaves a variable shared by two `OPTIONAL`s unbound in
 * the second.
 *
 * A creator is labelled three ways. A `cdm:court_national` carries its name on
 * `cdm:court_national_name`. A corporate-body authority code resolves through
 * `resolveCorporateBodyLabel`. Any other CELLAR resource is dropped: on CELLAR these
 * are the `cdm:person` creators of case law, and each is also that work's Advocate
 * General, so it reaches the output through `advocatesGeneral` instead of as a UUID.
 */
export async function fetchWorkAgents(
  ...
  const query = `
SELECT ?agent ?role ?personName ?courtName WHERE {
  ...
  for (const b of await svc.query(query, ctx)) {
    ...
    if (courtName) institutions.add(courtName);
      ...
      institutions.add(resolveCorporateBodyLabel(agent));
    }
  }
  return {
